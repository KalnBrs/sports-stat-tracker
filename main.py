...
#For AP Project
# When adding a new team and team isn't found ask if they want to add it 
# When add stats if no number added tell them to add number
...
try:
    ...
    def check_team(self, teamH, teamA):
      ...
      while check_data_loop is True:
        ...
        if teamH in data["teams"] and teamA in data["teams"]:
          #stops loop and returns true
          check_data_loop = False
          return True
        elif teamH not in data["teams"] and teamA not in data["teams"]:
          #stops loop adds teams and returns true
          check_data_loop = False
          self.add_team(teamA)
          self.add_team(teamH)
          return True
  ...
  class coach_window(tk.Toplevel):
    def __init__(self):
      ...
      self.button_coach = ttk.Button(self, text="Search", command=self.coach_add).pack(anchor=tk.W, padx=10)
  ...
  label_start = ttk.Label(text="Welcome to the Basketball Stat Tracker").pack(padx=10)
  label_start_ask = ttk.Label(text="Do you want to start or search for a player or team?").pack(padx=10, pady=10)
